...
const repeatString = (string, number, separator) => new Array(number).fill(string).join(separator)
...
const checkStart = (str, search) => str.toLowerCase().startsWith(search.toLowerCase())
...
const setUpperCase = (str) =>
    str.toLowerCase().split(' ').map(el => el[0].toUpperCase() + el.slice(1)).join(' ')

//ucFirst("вася")str.toLowerCase().split(' ').map(el => ucFirst(el)).join(' ')
...
const isIncludes = (str, search) => search.toLowerCase().split('').every(el => str.toLowerCase().includes(el))
